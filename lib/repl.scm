  ...
  (define *repl*                    ; environment that holds the REPL functions
	...
	(let ((histbuf (make-vector 100 ""))
	  ...
	  (define* (write-history port (spaces 0))
	    ...
	    (let-temporarily (((*shack* 'print-length) (* 2 histsize)))
			 ...
			 (catch #t
			   ...
			   (lambda (type info)
			     (with-let (unlet)
			       ...
			       (let ((op (*shack* 'print-length)))
				 (if (< op 32) (set! (*shack* 'print-length) 32))
				 (if (and (pair? info)
					  (string? (car info)))
				     (format *stderr* " ~A" (apply format #f info))
				     (if (not (null? info))
					 (format *stderr* " ~A" info)))
				 (if (< op 32) (set! (*shack* 'print-length) op)))
			       (with-repl-let
				(lambda ()
...
;;; to display a variable's value as shack runs using the repl help window:
